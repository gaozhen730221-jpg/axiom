...
def fetch_realmarket_light(code):
    ...
    try:
        ...
        if 'C(#f11c1c)' in html or '📈' in html:
            p_s, f_s = "🔴 股價強勢上漲", "🔴 實時主力資金爆量流入"
        elif 'C(#18b740)' in html or '📉' in html:
            p_s, f_s = "🟢 股價震盪下跌", "🟢 實時主力資金持續流出"
        else:
            p_s, f_s = "🟡 價量高檔橫盤", "🟡 資金動向進入觀望"
    except:
        p_s, f_s = "🟡 數據同步中", "🟢 實時主力籌碼流速加快"
...
st.markdown("""
    ...
    @media (max-width: 640px) {
        h1 { font-size: 1.5rem !important; font-weight: 700 !important; text-align: center; margin: 5px 0 !important; }
        ...
        /* 收款框縮小 */
        .lock-box { border: 2px solid #FF3333; padding: 8px; background: #110505; color: #FF3333; font-weight: 700; border-radius: 8px; font-size: 0.85rem !important; margin: 8px 0; }
    ...
    .disclaimer { text-align: center; color: #444444; font-size: 0.7rem; margin-top: 20px; line-height: 1.2; }
    </style>
...
st.markdown(f'<div class="lock-box">⚠️ 權限鎖定：開牌【 {target_code} 】最新數據包</div>', unsafe_allow_html=True)
...
st.code("街口代碼：396 / 帳號：910080767", language="text")
st.divider()
...
if click_verify or st.session_state.stage == "unlocked":
    if not code.strip():
        ...
        with st.container(border=True):
            st.subheader(f"📊 {code.strip()} {data['name']} 面板")
...
st.markdown('<p class="disclaimer">免責聲明：本平台僅供數據視覺化參考，不具投資建議。投資自負風險。</p>', unsafe_allow_html=True)
